...
COPY /workspace/.codesandbox/supervisord.conf /etc/supervisor/conf.d/supervisord.conf
COPY /workspace/.codesandbox/nginx.conf /etc/nginx/nginx.conf

RUN mkdir /etc/mysql /usr/local/mysql
COPY /workspace/.codesandbox/config.json /etc/mysql/
COPY /workspace/.codesandbox/entrypoint.sh /usr/local/mysql/
